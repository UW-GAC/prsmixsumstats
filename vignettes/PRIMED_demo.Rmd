...
```{r}
## setup grid of penalty parameters (alpha and lambda)
...
## penalty factor should be 0 for covariates and 1 for PRS
penalty_factor <- rep(0,ncol(sumstats$xx))
penalty_factor[grepl("^PRS", colnames(sumstats$xx))] <- 1
...
```{r}
metrics_obs <- metrics_sumstats(sumstats, fit_obs, penalty_factor, trait_type = "binary")
